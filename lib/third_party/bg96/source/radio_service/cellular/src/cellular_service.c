/**
  ******************************************************************************
  * @file   Middlewares\ST\radio_service\cellular\src\cellular_service.c
...
#define PrintINFO(format, args...)  vLoggingPrintf("CS:" format "\r\n", ## args)
#define PrintDBG(format, args...)   vLoggingPrintf("CS:" format "\r\n", ## args)
#define PrintAPI(format, args...)   vLoggingPrintf("CS API:" format "\r\n", ## args)
#define PrintErr(format, args...)   vLoggingPrintf("CS ERROR:" format "\r\n", ## args)
#else
...
static __IO uint8_t eventReceived = 0;
/* URC callbacks */
...
static cellular_pdn_event_callback_t urc_packet_domain_event_callback[CS_PDN_CONFIG_MAX+1] = {NULL};
...
static CS_ModemConfig_t cs_ctxt_modem_config;
...
static void socket_init(uint8_t index);
...
/* static CS_Bool_t socket_is_configured(socket_handle_t sockhandle); TODO ? */
/* static void socket_release(socket_handle_t sockhandle); TODO ?*/
...
/**
  ...
  *
  * This function has to be called peridiocally by the client in bare mode.
  *
...
/**
  ...
  *
  * This function is used to init Cellular Service and also set the event
  * callback which is used in bare mode only.
  *
  * !!! IMPORTANT !!! in Bare mode, event_callback will be call under interrupt
...
/**
  ...
  *
  * This function is used to configure persitent modem configuration
...
CS_Status_t CS_modem_config(CS_ModemConfig_t *p_config)
{
    ...
    at_status_t err;
    PrintAPI("CS_modem_config");

    /* TODO : may be use with CS_define_pdn */
    memcpy((void *)&cs_ctxt_modem_config, (void *)p_config, sizeof(CS_ModemConfig_t));
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_MODEMCONFIG, sizeof(CS_ModemConfig_t), (void *)&cs_ctxt_modem_config) == DATAPACK_OK)
    {
        err = AT_sendcmd(_Adapter_Handle, SID_CS_MODEM_CONFIG, &cmd_buf[0], &rsp_buf[0]);
        if (err == ATSTATUS_OK)
        {
            PrintDBG("<Cellular_Service> Modem configured");
            retval = CELLULAR_OK;
        }
    }

    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during modem config"); }
    return(retval);
}
...
CS_Status_t CS_power_on(void)
{
    ...
    if (SysCtrl_power_on(DEVTYPE_MODEM_CELLULAR) == SCSTATUS_OK)
    {
        ...
        if(DATAPACK_writeStruct(&cmd_buf[0], CSMT_NONE, 0, NULL) == DATAPACK_OK)
        {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when power on process"); }
    return(retval);
}
...
CS_Status_t CS_power_off(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during power off process"); }
    return(retval);
}
...
/**
  ...
  *
  * Usually, the command AT is sent and OK is expected as response
  *
  * @param  none
  * @retval CS_Status_t
  */
CS_Status_t CS_check_connection(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error with modem connection"); }
    return(retval);
}
...
/**
  ...
  *
  * Used to provide PIN code (if any) and modem function level.
  *
...
CS_Status_t CS_init_modem(CS_ModemInit_t init, CS_Bool_t reset, const CS_CHAR_t *pin_code)
{
    ...
    memcpy((void *)&modemInit_struct.pincode.pincode[0], (void *)pin_code, strlen((const char *)pin_code));

    if (DATAPACK_writeStruct(&cmd_buf[0], CSMT_INITMODEM, sizeof(csint_modemInit_t), (void *)&modemInit_struct) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during init"); }
    return(retval);
}
...
CS_Status_t CS_get_device_info(CS_DeviceInfo_t *p_devinfo)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when getting device infos"); }
    return(retval);
}
...
/**
  ...
  *
  * This function is used to select the operator. It returns a detailled
                            ...
                            CS_RegistrationStatus_t *p_reg_status)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CSMT_OPERATORSELECT, sizeof(CS_OperatorSelector_t), (void *)&cs_ctxt_operator) == DATAPACK_OK)
    {
        err = AT_sendcmd(_Adapter_Handle, SID_CS_REGISTER_NET, &cmd_buf[0], &rsp_buf[0]);
        if (err == ATSTATUS_OK)
        {
            if (DATAPACK_readStruct(&rsp_buf[0], CSMT_REGISTRATIONSTATUS, sizeof(CS_RegistrationStatus_t), p_reg_status) == DATAPACK_OK)
            {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during network registration "); }
    return(retval);
}
...
/**
  ...
  *
  * This function should be called for each event the user wants to start monitoring.
  *
...
CS_Status_t CS_subscribe_net_event(CS_UrcEvent_t event, cellular_urc_callback_t urc_callback)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when subscribe event"); }
    return(retval);
}
...
/**
  ...
  *
  * This function should be called for each event the user wants to stop monitoring.
  *
...
CS_Status_t CS_unsubscribe_net_event(CS_UrcEvent_t event)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when unsubscribe event"); }
    return(retval);
}
...
CS_Status_t CS_attach_PS_domain(void)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_ATTACH_PS_DOMAIN, 0, NULL) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when attaching PS domain"); }
    return(retval);
}
...
CS_Status_t CS_detach_PS_domain(void)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_DETACH_PS_DOMAIN, 0, NULL) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when detaching PS domain"); }
    return(retval);
}
...
CS_Status_t CS_get_attach_status(CS_PSattach_t *p_attach)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when getting attachment status"); }
    return(retval);
}
...
CS_Status_t CS_get_net_status(CS_RegistrationStatus_t *p_reg_status)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when getting net status"); }
    return(retval);
}
...
CS_Status_t CS_get_signal_quality(CS_SignalQuality_t *p_sig_qual)
{
    ...
    if (DATAPACK_writePtr(&cmd_buf[0], CMST_SIGNAL_QUALITY, (void *)&local_sig_qual) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when getting signal quality"); }
    return(retval);
}
...
/**
  ...
  *
  * This function triggers the allocation of IP public WAN to the device.
  * Note: only one PDN can be activated at a time.
  *
  * @param  cid Configuration identifier
  *   This parameter can be one of the following values:
  *     @arg CS_PDN_PREDEF_CONFIG To use default PDN configuration.
  *     @arg CS_PDN_USER_CONFIG_1-5 To use a dedicated PDN configuration.
  *     @arg CS_PDN_CONFIG_DEFAULT To use default PDN config set by CS_set_default_pdn().
  * @retval CS_Status_t
  */
CS_Status_t CS_activate_pdn(CS_PDN_conf_id_t cid)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_ACTIVATE_PDN, sizeof(CS_PDN_conf_id_t), (void *)&cid) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when PDN %cid activation", cid); }
    return(retval);
}
...
CS_Status_t CS_deactivate_pdn(CS_PDN_conf_id_t cid)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_DEACTIVATE_PDN, sizeof(CS_PDN_conf_id_t), (void *)&cid) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when PDN %cid deactivation", cid); }
    return(retval);
}
...
CS_Status_t CS_define_pdn(CS_PDN_conf_id_t cid, const CS_CHAR_t *apn, CS_PDN_configuration_t *pdn_conf)
{
    ...
    memcpy((void *)&pdn_infos.apn[0], (void *)apn, strlen((const char *)apn));
    memcpy((void *)&pdn_infos.pdn_conf, (void *)pdn_conf, sizeof(CS_PDN_configuration_t));

    if (DATAPACK_writePtr(&cmd_buf[0], CMST_DEFINE_PDN, (void *)&pdn_infos) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when defining PDN %d", cid); }
    return(retval);
}
...
/**
  ...
  *
  * By default, PDN_PREDEF_CONFIG is considered as the default PDN.
  *
  * @param  cid Configuration identifier.
  * @retval CS_Status_t
  */
CS_Status_t CS_set_default_pdn(CS_PDN_conf_id_t cid)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_SET_DEFAULT_PDN, sizeof(CS_PDN_conf_id_t), (void *)&cid) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when setting default PDN %d", cid); }
    return(retval);
}
...
CS_Status_t CS_get_dev_IP_address(CS_PDN_conf_id_t cid, CS_IPaddrType_t *ip_addr_type, CS_CHAR_t *p_ip_addr_value)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_GET_IP_ADDRESS, sizeof(CS_PDN_conf_id_t), (void *)&cid) == DATAPACK_OK)
    {
        err = AT_sendcmd(_Adapter_Handle, SID_CS_GET_IP_ADDRESS, &cmd_buf[0], &rsp_buf[0]);
        if (err == ATSTATUS_OK)
        {
            csint_ip_addr_info_t ip_addr_info;
            if (DATAPACK_readStruct(&rsp_buf[0], CMST_GET_IP_ADDRESS, sizeof(csint_ip_addr_info_t), &ip_addr_info) == DATAPACK_OK)
            {
                PrintDBG("<Cellular_Service> IP address informations received");
                /* recopy info to user */
                *ip_addr_type = ip_addr_info.ip_addr_type;
                memcpy((void*)p_ip_addr_value, (void*)&ip_addr_info.ip_addr_value, strlen((const char *)ip_addr_info.ip_addr_value));
                PrintDBG("<Cellular_Service> IP address = %s (type = %d)", (CS_CHAR_t *)ip_addr_info.ip_addr_value, ip_addr_info.ip_addr_type );
                retval = CELLULAR_OK;
            }
        }
    }

    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when getting IP address informations"); }
    return(retval);
}
/**
  * @brief  Initialization of Cellular Service in bare mode
...
/**
  ...
  *
  * This function is used to register to an event related to a PDN
...
CS_Status_t  CS_register_pdn_event(CS_PDN_conf_id_t cid, cellular_pdn_event_callback_t pdn_event_callback)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service>error when registering PDN events"); }
    return(retval);
}
...
CS_Status_t CS_deregister_pdn_event(CS_PDN_conf_id_t cid)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when deregistering PDN events"); }
    return(retval);
}
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg IPAT_IPV4 for IPV4 (default)
  *     @arg IPAT_IPV6  for IPV6
                                  ...
                                  CS_PDN_conf_id_t cid)
{
    ...
    int8_t sockhandle = socket_allocateHandle();
    ...
    if(socket_create(sockhandle, addr_type, protocol, local_port, cid) != CELLULAR_OK)
    {
        /* deallocate handle */
...
/**
  ...
  *
  * This function has to be called before to use a socket.
  *
                                       ...
                                       cellular_socket_closed_callback_t remote_close_cb)
{
    ...
    PrintDBG("DBG: socket data ready callback=0x%x", cs_ctxt_sockets_info[sockHandle].socket_data_ready_callback);
    ...
    PrintDBG("DBG: socket remote closed callback=0x%x", cs_ctxt_sockets_info[sockHandle].socket_remote_close_callback);
    ...
    /* TODO : use data sent callback only for asynchronous (NOT IMPLEMENTED) */
...
/**
  ...
  *
  * This function is called to configure one parameter at a time.
  ...
  *     @arg SOL_TRANSPORT
  * @param  opt_name
  *     @arg SON_IP_MAX_PACKET_SIZE Maximum packet size for data transfer (0 to 1500 bytes).
  *     @arg SON_TRP_MAX_TIMEOUT Inactivity timeout (0 to 65535, in second, 0 means infinite).
  *     @arg SON_TRP_CONNECT_SETUP_TIMEOUT Maximum timeout to setup connection with remote server
  *                                       (10 to 1200, in 100 of ms, 0 means infinite).
  *     @arg SON_TRP_TRANSFER_TIMEOUT Maximum timeout to transfer data to remote server
  *                                   (1 to 255, in ms, 0 means infinite).
  *     @arg SON_TRP_CONNECT_MODE To indicate if connection with modem will be dedicated (CM_ONLINE_MODE)
  ...
  *     @arg SON_TRP_SUSPEND_TIMEOUT To define inactivty timeout to suspend online mode
  *                                  (0 to 2000, in ms , 0 means infinite).
  *     @arg SON_TRP_RX_TIMEOUT Maximum timeout to receive data from remoteserver (0 to 255, in ms, 0 means infinite).
...
/**
  ...
  *
  * This function is called for one parameter at a time.
  *
...
/**
  ...
  *
  * This function is blocking until the connection is setup or when the timeout to wait for socket
  * connection expires.
  *
  * @param  sockHandle Handle of the socket.
    ...
    if (retval == CELLULAR_OK)
    {
        ...
        */
        ...
        if (DATAPACK_writePtr(&cmd_buf[0], CSMT_SOCKET_INFO, (void *)socket_infos) == DATAPACK_OK)
        {
            ...
            {
                /* NOT SUPPORTED YET */
                /* TODO SID_CS_DIAL_ONLINE_SUSP to manage also ? */
...
/**
  ...
  *
  * This function is blocking until the data is transfered or when the timeout to wait for transmission expires.
  *
  * @param  sockHandle Handle of the socket
                                 ...
                                 uint32_t length)
{
    ...
    {
        PrintErr("<Cellular_Service> buffer size %d exceed maximum value %d",
                 length,
    ...
    send_data_struct.p_buffer_addr = p_buf;
    send_data_struct.buffer_size = length;
    send_data_struct.max_buffer_size = length;
    if (DATAPACK_writeStruct(&cmd_buf[0], CSMT_SOCKET_DATA_BUFFER, sizeof(csint_socket_data_buffer_t), (void *)&send_data_struct) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when sending data to socket"); }
    return(retval);
}
...
/**
  ...
  *
  * This function is blocking until expected data length is received or a receive timeout has expired.
  *
  * @param  sockHandle Handle of the socket
  ...
  */
int CDS_socket_receive(socket_handle_t sockHandle,
                                    CS_CHAR_t *p_buf,
                                    uint32_t max_buf_length)
{
    ...
    receive_data_struct.p_buffer_addr = p_buf;
    receive_data_struct.buffer_size = 0;
    receive_data_struct.max_buffer_size = max_buf_length;
    if (DATAPACK_writeStruct(&cmd_buf[0], CSMT_SOCKET_DATA_BUFFER, sizeof(csint_socket_data_buffer_t), (void *)&receive_data_struct) == DATAPACK_OK)
    {
...
/**
  ...
  *
  * If a PDN is activated at socket creation, the socket will not be deactivated at socket closure.
  *
  * @param  sockHandle Handle of the socket
...
{
    UNUSED(force); /* TODO parameter force not used ? */
    CS_Status_t retval = CELLULAR_ERROR;
    at_status_t err;
    ...
    switch (cs_ctxt_sockets_info[sockHandle].state)
    {
     ...
     case SOCKETSTATE_CONNECTED:
        /* continue below and send request to the modem */
        break;
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when closing socket"); }
    return(retval);
}
                                  ...
                                  CS_SocketCnxInfos_t *infos)
{
    ...
    socket_cnx_infos.infos = infos;
    memset((void *)infos, 0, sizeof(CS_SocketCnxInfos_t));
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when requesting socket cnx status"); }
    return(retval);
}
...
CS_Status_t CS_suspend_data(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when suspending DATA"); }
    return(retval);
}
...
CS_Status_t CS_resume_data(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error when resuming DATA"); }
    return(retval);
}
...
static CS_Status_t perform_HW_reset(void)
{
    ...
    if (DATAPACK_writeStruct(&cmd_buf[0], CMST_RESET, sizeof(CS_Reset_t), (void *)&rst_type) == DATAPACK_OK)
    {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during HW device reset"); }
    return(retval);
}
...
static CS_Status_t perform_SW_reset(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during SW device reset"); }
    return(retval);
}
...
static CS_Status_t perform_Factory_reset(void)
{
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("<Cellular_Service> error during Factory device reset"); }
    return(retval);
}
...
/**
  * @brief  DNS request
  *         Configure DNS and get IP address of the specifed hostname
  *
...
CS_Status_t CS_dns_request(CS_PDN_conf_id_t cid, CS_DnsReq_t *dns_req, CS_DnsResp_t *dns_resp)
{
    ...
    memcpy((void *)&loc_dns_req.dns_request, dns_req, sizeof(CS_DnsReq_t));

    if (DATAPACK_writePtr(&cmd_buf[0], CMST_DNS_REQ, (void *)&loc_dns_req) == DATAPACK_OK)
    {
        err = AT_sendcmd(_Adapter_Handle, SID_CS_DNS_REQ, &cmd_buf[0], &rsp_buf[0]);
        if (err == ATSTATUS_OK)
        {
            if (DATAPACK_readStruct(&rsp_buf[0], CMST_DNS_REQ, sizeof(dns_resp->host_addr), dns_resp->host_addr) == DATAPACK_OK)
            {
    ...
    if (retval == CELLULAR_ERROR) { PrintErr("Error during DNS request"); }
    return(retval);
}
...
static void CELLULAR_reset_context(void)
{
    ...
    cs_ctxt_eps_location_info.ci = 0;
    cs_ctxt_eps_location_info.lac = 0;
    ...
    cs_ctxt_gprs_location_info.ci = 0;
    cs_ctxt_gprs_location_info.lac = 0;
    ...
    cs_ctxt_cs_location_info.ci = 0;
    cs_ctxt_cs_location_info.lac = 0;
...
{
    uint8_t cpt;
    for(cpt =0; cpt < CELLULAR_MAX_SOCKETS; cpt ++)
    {
        socket_init(cpt);
    }
}
...
static void CELLULAR_urc_notif(at_buf_t *p_rsp_buf)
{
    ...
    if ((msgtype == CMST_URC_EPS_NETWORK_REGISTRATION_STATUS) &&
        (cs_ctxt_urc_subscription.eps_network_registration == CELLULAR_TRUE))
    {
        CS_NetworkRegState_t rx_state;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_EPS_NETWORK_REGISTRATION_STATUS,
    ...
    else if ((msgtype == CMST_URC_GPRS_NETWORK_REGISTRATION_STATUS) &&
        (cs_ctxt_urc_subscription.gprs_network_registration == CELLULAR_TRUE))
    {
        CS_NetworkRegState_t rx_state;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_GPRS_NETWORK_REGISTRATION_STATUS,
                                sizeof(CS_NetworkRegState_t), (void *)&rx_state) == DATAPACK_OK)
        {
            /* if network registration status has changed, notify client */
    ...
    else if ((msgtype == CMST_URC_CS_NETWORK_REGISTRATION_STATUS) &&
        (cs_ctxt_urc_subscription.cs_network_registration == CELLULAR_TRUE))
    {
        CS_NetworkRegState_t rx_state;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_CS_NETWORK_REGISTRATION_STATUS,
                                sizeof(CS_NetworkRegState_t), (void *)&rx_state) == DATAPACK_OK)
        {
            /* if network registration status has changed, notify client */
    ...
    else if ((msgtype == CMST_URC_EPS_LOCATION_INFO) &&
             (cs_ctxt_urc_subscription.eps_location_info == CELLULAR_TRUE))
    {
        CS_Bool_t loc_update = CELLULAR_FALSE;
        csint_location_info_t rx_loc;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_EPS_LOCATION_INFO,
    ...
    else if ((msgtype == CMST_URC_GPRS_LOCATION_INFO) &&
             (cs_ctxt_urc_subscription.gprs_location_info == CELLULAR_TRUE))
    {
        CS_Bool_t loc_update = CELLULAR_FALSE;
        csint_location_info_t rx_loc;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_GPRS_LOCATION_INFO,
                                sizeof(csint_location_info_t), (void *)&rx_loc) == DATAPACK_OK)
        {
            /* ci received and changed since last time ? */
            if (rx_loc.ci_updated == CELLULAR_TRUE)
    ...
    else if ((msgtype == CMST_URC_CS_LOCATION_INFO) &&
             (cs_ctxt_urc_subscription.cs_location_info == CELLULAR_TRUE))
    {
        CS_Bool_t loc_update = CELLULAR_FALSE;
        csint_location_info_t rx_loc;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_CS_LOCATION_INFO,
                                sizeof(csint_location_info_t), (void *)&rx_loc) == DATAPACK_OK)
        {
            /* ci received and changed since last time ? */
            if (rx_loc.ci_updated == CELLULAR_TRUE)
    ...
    /* --- SIGNAL QUALITY URC --- */
    else if ((msgtype == CMST_URC_SIGNAL_QUALITY) &&
             (cs_ctxt_urc_subscription.signal_quality == CELLULAR_TRUE))
    {
        CS_SignalQuality_t local_sig_qual;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_SIGNAL_QUALITY,
    ...
    else if (msgtype == CSMT_URC_SOCKET_DATA_PENDING)
    {
      ...
      if (DATAPACK_readStruct(&p_rsp_buf[0], CSMT_URC_SOCKET_DATA_PENDING,
    ...
    else if (msgtype == CSMT_URC_SOCKET_CLOSED)
    {
        /* unpack datas received */
        socket_handle_t sockHandle;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CSMT_URC_SOCKET_CLOSED,
                                sizeof(socket_handle_t), (void *)&sockHandle) == DATAPACK_OK)
        {
            if (sockHandle != CS_INVALID_SOCKET_HANDLE)
            {
    ...
    else if ((msgtype == CMST_URC_PACKET_DOMAIN_EVENT) &&
             (cs_ctxt_urc_subscription.packet_domain_event == CELLULAR_TRUE))
    {
        /* unpack datas received */
        csint_PDN_event_desc_t pdn_event;
        if (DATAPACK_readStruct(&p_rsp_buf[0], CMST_URC_PACKET_DOMAIN_EVENT,
                                sizeof(csint_PDN_event_desc_t), (void *)&pdn_event) == DATAPACK_OK)
        {
            ...
            else if (pdn_event.conf_id == CS_PDN_ALL)
            {
                ...
                for (uint8_t loop=0; loop<=CS_PDN_CONFIG_MAX; loop++)
                {
...
static void socket_init(uint8_t index)
{
    ...
    cs_ctxt_sockets_info[index].local_port = 0;
                                         ...
                                         void *p_opt_val)
{
  ...
  /* TODO check parameters validity (timeout values do not exceed limit, 0 = default value (set explicitly) */
  ...
  if (opt_level == CS_SOL_IP)
  {
    ...
    case CS_SON_IP_MAX_PACKET_SIZE:
        p_uint16 = p_opt_val;
        if (*p_uint16 <= 1500) /* TODO why 1500 ? change hard-coded */
        {
            if (*p_uint16 == 0)
            {
