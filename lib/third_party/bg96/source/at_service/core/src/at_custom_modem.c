/**
  ******************************************************************************
  * @file    Middlewares\ST\at_service\core\src\at_custom_modem.c
...
#define MATCHING_USER_AND_MODEM_CID (1)

#if (USE_TRACE_INTERFACE_ATCUSTOM_MODEM == 1)
...
#define PrintBuf(format, args...)   do {} while(0) /* Cannot print a buffer using printf */
#elif (USE_CONFIGPRINTF_ATCUSTOM_MODEM == 1)
#define PrintINFO(format, args...)  vLoggingPrintf("ATCModem:" format "\r\n", ## args)
#define PrintDBG(format, args...)   vLoggingPrintf("ATCModem:" format "\r\n", ## args)
#define PrintAPI(format, args...)   vLoggingPrintf("ATCModem:" format "\r\n", ## args)
#define PrintErr(format, args...)   vLoggingPrintf("ATCModem:" format "\r\n", ## args)
...
static void reserve_modem_cid(atcustom_persistent_context_t *p_persistent_ctxt, CS_PDN_conf_id_t conf_id, uint8_t reserved_modem_cid)
{
#if (MATCHING_USER_AND_MODEM_CID == 0)
...
*
*/
CS_PDN_conf_id_t find_user_cid_with_matching_ip_addr(atcustom_persistent_context_t *p_persistent_ctxt, csint_ip_addr_info_t *ip_addr_struct)
{
    CS_PDN_conf_id_t user_cid = CS_PDN_NOT_DEFINED;

    /* seach user config ID corresponding to this IP address */
    for (uint8_t loop=0; loop<MODEM_MAX_NB_PDP_CTXT; loop++)
    {
        ...
        /* size not really needed but added for robustness
           to implement: a function which compare IP address without taking into account special
           characters (quote, space, ...) */
        uint8_t size1, size2, minsize;
        size1 = strlen((char *)&ip_addr_struct->ip_addr_value);
        size2 = strlen((char *)&p_persistent_ctxt->modem_cid_table[loop].ip_addr_infos.ip_addr_value);
...
void atcm_put_IP_address_infos(atcustom_persistent_context_t *p_persistent_ctxt, uint8_t modem_cid, csint_ip_addr_info_t *ip_addr_info)
{
    for (uint8_t i=0; i< MODEM_MAX_NB_PDP_CTXT; i++)
    {
        if (p_persistent_ctxt->modem_cid_table[i].cid_value == modem_cid)
        {
            ...
            memcpy((void *)& p_persistent_ctxt->modem_cid_table[i].ip_addr_infos, (void *)ip_addr_info, sizeof(csint_ip_addr_info_t));
            return;
        }
    }
...
void atcm_get_IP_address_infos(atcustom_persistent_context_t *p_persistent_ctxt, CS_PDN_conf_id_t conf_id, csint_ip_addr_info_t  *ip_addr_info)
{
    for (uint8_t i=0; i< MODEM_MAX_NB_PDP_CTXT; i++)
    {
        if (p_persistent_ctxt->modem_cid_table[i].affected_config == conf_id)
        {
            ...
            memcpy((void *)ip_addr_info, (void *)& p_persistent_ctxt->modem_cid_table[i].ip_addr_infos, sizeof(csint_ip_addr_info_t));
            return;
        }
    }
...
uint32_t atcm_socket_get_modem_cid(atcustom_modem_context_t *p_modem_ctxt, socket_handle_t sockHandle)
{
    ...
    cid = p_modem_ctxt->persist.socket[sockHandle].socket_connId_value;
...
socket_handle_t atcm_socket_get_socket_handle(atcustom_modem_context_t *p_modem_ctxt, uint32_t modemCID)
{
    ...
    for (uint8_t i=0; i< CELLULAR_MAX_SOCKETS; i++)
    {
    ...
    if (sockHandle == CS_INVALID_SOCKET_HANDLE)
    {
        PrintINFO("Can not find valid socket handle for modem CID=%d", modemCID);
    }
...
AT_CHAR_t* atcm_get_CmdStr(atcustom_modem_context_t *p_modem_ctxt, uint32_t cmd_id)
{
    ...
    for(i=0; i< p_modem_ctxt->modem_LUT_size; i++)
    {
        if (p_modem_ctxt->p_modem_LUT[i].cmd_id == cmd_id)
        {
            return (AT_CHAR_t *)(&p_modem_ctxt->p_modem_LUT[i].cmd_str);
        }
    }
...
uint32_t atcm_get_CmdTimeout(atcustom_modem_context_t *p_modem_ctxt, uint32_t cmd_id)
{
    ...
    /* should not fall here - return default value */
    PrintErr("INVALID CMD ID %d (CmdTimeout)", cmd_id);
    return (MODEM_DEFAULT_TIMEOUT);
}
...
CmdBuildFuncTypeDef atcm_get_CmdBuildFunc(atcustom_modem_context_t *p_modem_ctxt, uint32_t cmd_id)
{
    ...
    /* should not fall here - return default value */
    PrintErr("INVALID CMD ID (CmdBuildFunc)");
...
CmdAnalyzeFuncTypeDef atcm_get_CmdAnalyzeFunc(atcustom_modem_context_t *p_modem_ctxt, uint32_t cmd_id)
{
    ...
    /* should not fall here - return default value */
    PrintErr("INVALID CMD ID (CmdAnalyzeFunc)");
...
AT_CHAR_t* atcm_get_PDPtypeStr(CS_PDPtype_t pdp_type)
{
    ...
    for (i=0; i<max_array_size; i++)
    {
        ...
        {
            return ((AT_CHAR_t *)(&ACTM_PDP_TYPE_LUT[i].pdp_type_string));
        }
    }
...
void atcm_program_AT_CMD (atcustom_modem_context_t *p_modem_ctxt, atparser_context_t *p_atp_ctxt, at_type_t cmd_type, uint32_t cmd_id, atcustom_FinalCmd_t final)
{
    ...
    /* is it final command ? */
    p_atp_ctxt->is_final_cmd =  (final == FINAL_CMD) ? 1 : 0;
...
at_status_t atcm_modem_build_cmd (atcustom_modem_context_t *p_modem_ctxt, atparser_context_t *p_atp_ctxt, uint32_t *p_ATcmdTimeout)
{
    ...
    sprintf((char *)p_atp_ctxt->current_atcmd.name , (char *)atcm_get_CmdStr(p_modem_ctxt, p_atp_ctxt->current_atcmd.id));
    ...
    *p_ATcmdTimeout = p_atp_ctxt->cmd_timeout;
    PrintDBG("==== CMD TIMEOUT = %d ====", *p_ATcmdTimeout);
    ...
    */
    ...
    switch (p_atp_ctxt->current_SID)
    {
     case SID_CS_GET_ATTACHSTATUS:
        ...
        if  (DATAPACK_writeStruct(&p_rsp_buf[0], CSMT_ATTACHSTATUS, sizeof(CS_PSattach_t), (void *)&p_modem_ctxt->SID_ctxt.attach_status) != DATAPACK_OK)
        {
     ...
     case SID_CS_RECEIVE_DATA:
        /* PACK data to response buffer */
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CSMT_SOCKET_RXDATA, sizeof(uint32_t), (void *)&p_modem_ctxt->socket_ctxt.socketReceivedata.buffer_size) != DATAPACK_OK)
        {
            PrintErr("Buffer size problem");
            retval = ATSTATUS_ERROR;
        }
     ...
     case SID_CS_GET_NETSTATUS:
        ...
        /* PACK data to response buffer */
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CSMT_REGISTRATIONSTATUS, sizeof(CS_RegistrationStatus_t), (void *)&p_modem_ctxt->SID_ctxt.read_operator_infos) != DATAPACK_OK)
        {
            PrintErr("Buffer size problem");
            retval = ATSTATUS_ERROR;
        }
     ...
     case SID_CS_GET_IP_ADDRESS:
        {
            ...
            /* PACK data to response buffer */
            if(DATAPACK_writeStruct(&p_rsp_buf[0], CMST_GET_IP_ADDRESS, sizeof(csint_ip_addr_info_t), (void *)&ip_addr_info) != DATAPACK_OK)
            {
                PrintErr("Buffer size problem");
                retval = ATSTATUS_ERROR;
            }
...
at_status_t atcm_modem_get_urc(atcustom_modem_context_t *p_modem_ctxt, atparser_context_t *p_atp_ctxt, at_buf_t *p_rsp_buf)
{
    ...
    */
    ...
    if (p_modem_ctxt->persist.urc_avail_eps_network_registration == AT_TRUE)
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_EPS_NETWORK_REGISTRATION_STATUS, sizeof(CS_NetworkRegState_t), (void *)&p_modem_ctxt->persist.eps_network_state) == DATAPACK_OK)
        {
            /* reset flag */
   ...
   else if (p_modem_ctxt->persist.urc_avail_gprs_network_registration == AT_TRUE)
    {
        ...
        csint_msgType_t urc_type = CMST_URC_GPRS_NETWORK_REGISTRATION_STATUS;
        if (DATAPACK_writeStruct(&p_rsp_buf[0], urc_type, sizeof(CS_NetworkRegState_t), (void *)&p_modem_ctxt->persist.gprs_network_state) == DATAPACK_OK)
        {
            /* reset flag */
    ...
    else if (p_modem_ctxt->persist.urc_avail_cs_network_registration == AT_TRUE)
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_CS_NETWORK_REGISTRATION_STATUS, sizeof(CS_NetworkRegState_t), (void *)&p_modem_ctxt->persist.cs_network_state) == DATAPACK_OK)
        {
            /* reset flag */
    ...
    else if((p_modem_ctxt->persist.urc_avail_eps_location_info_tac == AT_TRUE) || (p_modem_ctxt->persist.urc_avail_eps_location_info_ci == AT_TRUE))
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_EPS_LOCATION_INFO, sizeof(csint_location_info_t), (void *)&loc_struct) == DATAPACK_OK)
        {
            /* reset flags */
            if (p_modem_ctxt->persist.urc_avail_eps_location_info_tac == AT_TRUE)
            {
    ...
    else if((p_modem_ctxt->persist.urc_avail_gprs_location_info_lac == AT_TRUE) || (p_modem_ctxt->persist.urc_avail_gprs_location_info_ci == AT_TRUE))
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_GPRS_LOCATION_INFO, sizeof(csint_location_info_t), (void *)&loc_struct) == DATAPACK_OK)
        {
            /* reset flags */
            if (p_modem_ctxt->persist.urc_avail_gprs_location_info_lac == AT_TRUE)
            {
    ...
    else if((p_modem_ctxt->persist.urc_avail_cs_location_info_lac == AT_TRUE) || (p_modem_ctxt->persist.urc_avail_cs_location_info_ci == AT_TRUE))
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_CS_LOCATION_INFO, sizeof(csint_location_info_t), (void *)&loc_struct) == DATAPACK_OK)
        {
            /* reset flags */
            if (p_modem_ctxt->persist.urc_avail_cs_location_info_lac == AT_TRUE)
            {
    ...
    else if (p_modem_ctxt->persist.urc_avail_signal_quality == AT_TRUE)
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_SIGNAL_QUALITY, sizeof(CS_SignalQuality_t), (void *)&signal_quality_struct) == DATAPACK_OK)
        {
            /* reset flag */
    ...
    else if (p_modem_ctxt->persist.urc_avail_socket_data_pending == AT_TRUE)
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CSMT_URC_SOCKET_DATA_PENDING, sizeof(socket_handle_t), (void *)&sockHandle) == DATAPACK_OK)
        {
    ...
    else if (p_modem_ctxt->persist.urc_avail_socket_closed_by_remote == AT_TRUE)
    {
        ...
        if (DATAPACK_writeStruct(&p_rsp_buf[0], CSMT_URC_SOCKET_CLOSED, sizeof(socket_handle_t), (void *)&sockHandle) == DATAPACK_OK)
        {
            /* reset flag if no more socket data pending */
        ...
        if (p_modem_ctxt->persist.pdn_event.event_origine == CGEV_EVENT_ORIGINE_NW)
        {
            switch (p_modem_ctxt->persist.pdn_event.event_type)
            {
             case CGEV_EVENT_TYPE_DETACH:
                ...
                *  +CGEV: NW DETACH
                */
                if (DATAPACK_writeStruct(&p_rsp_buf[0], CMST_URC_PACKET_DOMAIN_EVENT, sizeof(csint_PDN_event_desc_t), (void *)&p_modem_ctxt->persist.pdn_event) != DATAPACK_OK)
                {
                    retval = ATSTATUS_ERROR;
                }
                break;
...
at_status_t atcm_unsubscribe_net_event(atcustom_modem_context_t *p_modem_ctxt, atparser_context_t *p_atp_ctxt)
{
    ...
    */
    ...
    if ((urcEvent == CS_URCEVENT_EPS_NETWORK_REG_STAT) || (urcEvent == CS_URCEVENT_EPS_LOCATION_INFO))
    {
        ...
        if (urcEvent == CS_URCEVENT_EPS_NETWORK_REG_STAT) {p_modem_ctxt->persist.urc_subscript_eps_networkReg = CELLULAR_FALSE; }
        if (urcEvent == CS_URCEVENT_EPS_LOCATION_INFO) {p_modem_ctxt->persist.urc_subscript_eps_locationInfo = CELLULAR_FALSE; }
...
void atcm_reset_persistent_context (atcustom_persistent_context_t *p_persistent_ctxt)
{
    ...
    for (uint8_t i=0; i< MODEM_MAX_NB_PDP_CTXT; i++)
    {
        p_persistent_ctxt->modem_cid_table[i].cid_value = i+1; /* modem cid value starting at 1 (0 is reserved) */
...
#if (MATCHING_USER_AND_MODEM_CID == 1)
    ...
    reserve_modem_cid(p_persistent_ctxt, CS_PDN_PREDEF_CONFIG, PDP_CONTEXT_DEFAULT_MODEM_CID);
    ...
    {
       memcpy((char *)&p_persistent_ctxt->pdp_ctxt_infos[CS_PDN_PREDEF_CONFIG].apn, &PDP_CONTEXT_DEFAULT_APN, sizeof(PDP_CONTEXT_DEFAULT_APN));
    }
    ...
    for (uint8_t i=0; i< CELLULAR_MAX_SOCKETS; i++)
    {
        p_persistent_ctxt->socket[i].socket_connId_value = i+1; /* socket ID range from 1 to 6, if a modem does not support this range => need to adapt */
...
void atcm_reset_CMD_context (atcustom_CMD_context_t *p_cmd_ctxt)
{
    ...
    p_cmd_ctxt->modem_cid = 0xFFFF;
    p_cmd_ctxt->baud_rate = MODEM_UART_BAUDRATE;
    p_cmd_ctxt->cfun_value = 1;
...
void atcm_reset_SOCKET_context (atcustom_modem_context_t *p_modem_ctxt)
{
    ...
    p_modem_ctxt->socket_ctxt.socket_current_connId = 0;
    p_modem_ctxt->socket_ctxt.socket_rx_expected_buf_size = 0;
    p_modem_ctxt->socket_ctxt.socket_rx_count_bytes_received = 0;
                                ...
                                at_element_info_t *element_infos)
{
    ...
    {
        /* if string length > 0 */
        if (strlen((const char *)(p_modem_ctxt->p_modem_LUT)[i].cmd_str) > 0)
        {
                                     ...
                                     at_element_info_t *element_infos)
{
    ...
    {
     case _AT_CGMI:
        if (fRspAnalyze_CGMI(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
     ...
     case _AT_CGMM:
        if (fRspAnalyze_CGMM(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
     ...
     case _AT_CGMR:
        if (fRspAnalyze_CGMR(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
     ...
     case _AT_CGSN:
        if (fRspAnalyze_CGSN(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
     ...
     case _AT_GSN:
        if (fRspAnalyze_GSN(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
     ...
     case _AT_IPR:
        if (fRspAnalyze_IPR(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
     ...
     case _AT_CIMI:
        if (fRspAnalyze_CIMI(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
     ...
     case _AT_CGPADDR:
        if (fRspAnalyze_CGPADDR(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos) != ATACTION_RSP_ERROR)
        {
            /* received a valid intermediate answer */
            retval = ATACTION_RSP_INTERMEDIATE;
        }
...
at_status_t atcm_retrieve_SID_parameters(atcustom_modem_context_t *p_modem_ctxt, atparser_context_t *p_atp_ctxt)
{
    ...
    if (p_atp_ctxt->step == 0)
    {
        ...
        case SID_CS_MODEM_CONFIG:
            ...
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_MODEMCONFIG, sizeof(CS_ModemConfig_t), (void *)&p_modem_ctxt->SID_ctxt.modem_config) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_GET_SIGNAL_QUALITY:
            /* retrieve pointer on client structure */
            if (DATAPACK_readPtr(p_atp_ctxt->p_cmd_input, CMST_SIGNAL_QUALITY, (void *)&p_modem_ctxt->SID_ctxt.signal_quality) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_RESET:
            /* retrieve client datas */
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_RESET, sizeof(CS_Reset_t), (void *)&p_modem_ctxt->SID_ctxt.reset_type) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_ACTIVATE_PDN:
            /* retrieve client datas */
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_ACTIVATE_PDN, sizeof(CS_PDN_conf_id_t), (void *)&p_modem_ctxt->SID_ctxt.pdn_conf_id) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_DEACTIVATE_PDN:
            /* retrieve client datas */
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_DEACTIVATE_PDN, sizeof(CS_PDN_conf_id_t), (void *)&p_modem_ctxt->SID_ctxt.pdn_conf_id) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_DEFINE_PDN:
            {
                ...
                /* retrieve pointer on client structure */
                if (DATAPACK_readPtr(p_atp_ctxt->p_cmd_input, CMST_DEFINE_PDN, (void *)&ptr_pdn_infos) != DATAPACK_OK)
                {
                    retval = ATSTATUS_ERROR;
                }
                else
                {
                    ...
                    memcpy((void *)&p_modem_ctxt->persist.pdp_ctxt_infos[ptr_pdn_infos->conf_id], (void *)ptr_pdn_infos, sizeof(csint_pdn_infos_t));
        ...
        case SID_CS_SET_DEFAULT_PDN:
            /* retrieve client datas */
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_SET_DEFAULT_PDN, sizeof(CS_PDN_conf_id_t), (void *)&p_modem_ctxt->persist.pdn_default_conf_id) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_GET_IP_ADDRESS:
            /* retrieve client datas */
            if (DATAPACK_readStruct((uint8_t *)p_atp_ctxt->p_cmd_input, CMST_GET_IP_ADDRESS, sizeof(CS_PDN_conf_id_t), (void *)&p_modem_ctxt->SID_ctxt.pdn_conf_id) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            break;
        ...
        case SID_CS_DNS_REQ:
            /* retrieve pointer on client structure */
            if (DATAPACK_readPtr(p_atp_ctxt->p_cmd_input, CMST_DNS_REQ, (void *)&p_modem_ctxt->SID_ctxt.dns_req) != DATAPACK_OK)
            {
                retval = ATSTATUS_ERROR;
            }
            else
            {
                /* set SID ctxt pdn conf id */
                p_modem_ctxt->SID_ctxt.pdn_conf_id = p_modem_ctxt->SID_ctxt.dns_req->conf_id;
            }
            break;
...
uint8_t atcm_get_affected_modem_cid(atcustom_persistent_context_t *p_persistent_ctxt, CS_PDN_conf_id_t conf_id)
{
    ...
    return(1); /* return first valid cid by default (do not return 0 which is a special cid value !!!) */
}
    ...
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD)
    {
        if (p_atp_ctxt->current_SID == SID_CS_INIT_MODEM)
        {
            ...
            */
            ...
            if (modemInit_struct->init == CS_CMI_MINI) { fun = 0; }
            else if (modemInit_struct->init == CS_CMI_FULL) { fun = 1; }
            else if (modemInit_struct->init == CS_CMI_SIM_ONLY) { fun = 4; }
            else
            {
            ...
            (modemInit_struct->reset == CELLULAR_TRUE) ? (rst = 1) : (rst = 0);
...
at_status_t fCmdBuild_ATD(atparser_context_t *p_atp_ctxt, atcustom_modem_context_t *p_modem_ctxt)
{
    ...
    {
        /* TODO PDP context number ? */
        /* sprintf((char *)p_atp_ctxt->current_atcmd.params , "*99#1"); */
...
at_status_t fCmdBuild_AT_AND_D(atparser_context_t *p_atp_ctxt, atcustom_modem_context_t *p_modem_ctxt)
{
    ...
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_EXECUTION_CMD)
    {
        /* set baud rate */
        sprintf((char *)p_atp_ctxt->current_atcmd.params , "0");
        /* TODO: hard-coded to 0 but should be selectable by user ? */
        /* sprintf((char *)p_atp_ctxt->current_atcmd.params , "%d", p_modem_ctxt->CMD_ctxt.dtr_value); */
                             ...
                             IPC_RxMessage_t *p_msg_in, at_element_info_t *element_infos)
{
    ...
    /* TODO use CmeErr function for the moment (only difference is that there is no CME_ERROR id */
    retval = fRspAnalyze_CmeErr(p_atp_ctxt, p_modem_ctxt, p_msg_in, element_infos);
                             ...
                             IPC_RxMessage_t *p_msg_in, at_element_info_t *element_infos)
{
    ...
    /* analyze parameters for +CME ERROR, depending of current command */
    switch(p_atp_ctxt->current_atcmd.id)
    {
      ...
      case _AT_CGSN:
        ...
        {
          case CGSN_SN:
            PrintErr("Modem Error for CGSN_SN, use unitialized value");
            memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.serial_number), '0', MAX_SIZE_SN);
            break;

          case CGSN_IMEI:
            PrintErr("Modem Error for CGSN_IMEI, use unitialized value");
            memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.imei), '0', MAX_SIZE_IMEI);
            break;

          case CGSN_IMEISV:
            PrintErr("Modem Error for CGSN_IMEISV, use unitialized value, parameter ignored");
            break;

          case CGSN_SVN:
            PrintErr("Modem Error for CGSN_SVN, use unitialized value, parameter ignored");
            break;

          default:
            PrintErr("Modem Error for CGSN, unexpected parameter");
      ...
      case _AT_CIMI:
        PrintErr("Modem Error for CIMI, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.imsi), '0', MAX_SIZE_IMSI);
        break;

      case _AT_CGMI:
        PrintErr("Modem Error for CGMI, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.manufacturer_name), '0', MAX_SIZE_MANUFACT_NAME);
        break;

      case _AT_CGMM:
        PrintErr("Modem Error for CGMM, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.model), '0', MAX_SIZE_MODEL);
        break;

      case _AT_CGMR:
        PrintErr("Modem Error for CGMR, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.revision), '0', MAX_SIZE_REV);
        break;

      case _AT_CNUM:
        PrintErr("Modem Error for CNUM, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.phone_number), '0', MAX_SIZE_PHONE_NBR);
        break;

      case _AT_GSN:
        PrintErr("Modem Error for GSN, use unitialized value");
        memset((void *)&(p_modem_ctxt->SID_ctxt.device_info->u.imei), '0', MAX_SIZE_IMEI);
        break;

        /* TODO consider all other error cases for AT commands
      case _AT_CPIN:
        etc...
        */

      default:
        PrintErr("Modem Error for cmd (id=%d)", p_atp_ctxt->current_atcmd.id);
        retval = ATACTION_RSP_ERROR;
        break;
    ...
    */
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD)
    {
        ...
        else if (element_infos->param_rank == 4)
        {
            ...
            memcpy((void *)&(p_modem_ctxt->SID_ctxt.read_operator_infos.operator_name[0]), (void *)&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
        ...
        else if (element_infos->param_rank == 5)
        {
            ...
            PrintDBG("+COPS: Access technology = %d", AcT );
    ...
    */
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD)
    {
        if (element_infos->param_rank == 2)
        {
            ...
            p_modem_ctxt->SID_ctxt.attach_status = (attach == 1) ? CS_PS_ATTACHED : CS_PS_DETACHED;
                             ...
                             IPC_RxMessage_t *p_msg_in, at_element_info_t *element_infos)
{
    ...
    PrintDBG("current cmd = %d", p_atp_ctxt->current_atcmd.id);
    PrintDBG("rank = %d",element_infos->param_rank);
    ...
    */
    if (p_atp_ctxt->current_atcmd.id == _AT_CREG)
    {
        ...
        if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD)
        {
            ...
            if (element_infos->param_rank == 3)
            {
                ...
                PrintDBG("+CREG: stat=%d", stat);
            }
            if (element_infos->param_rank == 4)
            {
                ...
                PrintDBG("+CREG: lac=%d", lac);
            }
            if (element_infos->param_rank == 5)
            {
                ...
                PrintDBG("+CREG: ci=%d", ci);
            }
            if (element_infos->param_rank == 6)
            {
                /* param traced only */
                PrintDBG("+CREG: act=%d",
                         ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
            }
            /* TODO: other parameters are not supported yet */
        }
    ...
    else
    {
        ...
        if (element_infos->param_rank == 4)
        {
            ...
            p_modem_ctxt->persist.urc_avail_cs_location_info_ci = AT_TRUE;
            p_modem_ctxt->persist.cs_location_info.ci = (uint16_t)ci;
            PrintDBG("+CREG URC: ci=%d", ci);
        }
        if (element_infos->param_rank == 5)
        {
            /* param traced only */
            PrintDBG("+CREG URC: act=%d",
                     ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
        }
    }
    ...
    */
    if (p_atp_ctxt->current_atcmd.id == _AT_CGREG)
    {
        ...
        if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD)
        {
            ...
            if (element_infos->param_rank == 3)
            {
                ...
                PrintDBG("+CGREG: stat=%d", stat);
            }
            if (element_infos->param_rank == 4)
            {
                uint32_t lac = ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
                p_modem_ctxt->persist.gprs_location_info.lac = (uint8_t)lac;
                PrintDBG("+CGREG: lac=%d", lac);
            }
            if (element_infos->param_rank == 5)
            {
                uint32_t ci = ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
                p_modem_ctxt->persist.gprs_location_info.ci = (uint16_t)ci;
                PrintDBG("+CGREG: ci=%d", ci);
            }
            if (element_infos->param_rank == 6)
            {
                /* param traced only */
                PrintDBG("+CGREG: act=%d",
                         ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
            }
            if (element_infos->param_rank == 7)
            {
                /* param traced only */
                PrintDBG("+CGREG: rac=%d",
                         ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
            }
    ...
    else
    {
        ...
        if (element_infos->param_rank == 2)
        {
            ...
            p_modem_ctxt->persist.urc_avail_gprs_network_registration = AT_TRUE;
            p_modem_ctxt->persist.gprs_network_state = convert_NetworkState(stat);
            PrintDBG("+CGREG URC: stat=%d", stat);
        }
        if (element_infos->param_rank == 3)
        {
            uint32_t lac = ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
        ...
        if (element_infos->param_rank == 4)
        {
            ...
            p_modem_ctxt->persist.urc_avail_gprs_location_info_ci = AT_TRUE;
            p_modem_ctxt->persist.gprs_location_info.ci = (uint16_t)ci;
            PrintDBG("+CGREG URC: ci=%d", ci);
        }
        if (element_infos->param_rank == 5)
        {
             /* param traced only */
            PrintDBG("+CGREG URC: act=%d",
                     ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
        }
        if (element_infos->param_rank == 6)
        {
            /* param traced only */
            PrintDBG("+CGREG URC: rac=%d",
                     ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
        }
    }
    ...
    */
    if (p_atp_ctxt->current_atcmd.id == _AT_CEREG)
    {
        ...
        if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD)
        {
            ...
            if (element_infos->param_rank == 4)
            {
                ...
                PrintDBG("+CEREG: tac=%d", tac);
            }
            if (element_infos->param_rank == 5)
            {
                uint32_t ci = ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
                p_modem_ctxt->persist.eps_location_info.ci = (uint16_t)ci;
                PrintDBG("+CEREG: ci=%d", ci);
            }
            if (element_infos->param_rank == 6)
            {
                /* param traced only */
                PrintDBG("+CEREG: act=%d",
                         ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
            }
            /* other parameters are not supported yet */
        }
    ...
    else
    {
        ...
        if (element_infos->param_rank == 2)
        {
            ...
            p_modem_ctxt->persist.eps_network_state = convert_NetworkState(stat);
            PrintDBG("+CEREG URC: stat=%d", stat);
        }
        if (element_infos->param_rank == 3)
        {
            uint32_t tac = ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size);
        ...
        if (element_infos->param_rank == 4)
        {
            ...
            p_modem_ctxt->persist.urc_avail_eps_location_info_ci = AT_TRUE;
            p_modem_ctxt->persist.eps_location_info.ci = (uint16_t)ci;
            PrintDBG("+CEREG URC: ci=%d", ci);
        }
        if (element_infos->param_rank == 5)
        {
            /* param traced only */
            PrintDBG("+CEREG URC: act=%d",
                     ATutil_convertStringToInt(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size));
        }
    }
...
#define MAX_CGEV_PARAM_SIZE (32)
        uint8_t copy_params[MAX_CGEV_PARAM_SIZE] = {0};
        char *found;
#define GET_MINIMUM_SIZE(a,b) (a<b?a:b)
        memcpy((void*)copy_params, (void*)&p_msg_in->buffer[element_infos->str_start_idx], GET_MINIMUM_SIZE(element_infos->str_size, MAX_CGEV_PARAM_SIZE));
#undef GET_MINIMUM_SIZE
        found = strtok((char *)copy_params, " ");
        while ( found  != NULL)
        {
                    ...
                    switch (p_modem_ctxt->persist.pdn_event.event_type)
                    {
                     ...
                     case CGEV_EVENT_TYPE_DEACTIVATION:
                        if (p_modem_ctxt->persist.pdn_event.event_scope == CGEV_EVENT_SCOPE_PDN)
                        {
                            ...
                            */
                            uint32_t cgev_cid = ATutil_convertStringToInt((uint8_t *)found, strlen(found));
                            p_modem_ctxt->persist.pdn_event.conf_id = atcm_get_configID_for_modem_cid(&p_modem_ctxt->persist, cgev_cid);
                            PrintDBG("+CGEV modem cid=%d (user conf Id =%d)", cgev_cid, p_modem_ctxt->persist.pdn_event.conf_id);
                        }
                        else
                        {
                            /* we are in the case:
            ...
            PrintDBG("(%d) ---> %s", strlen(found), (uint8_t *)found);
            found = strtok(NULL, " ");
        }
    ...
    else if (element_infos->param_rank == 4)
    {
            ...
            (p_modem_ctxt->persist.pdn_event.event_type == CGEV_EVENT_TYPE_DEACTIVATION))
        {
            ...
            * +CGEV: NW DEACT <PDP_type>, <PDP_addr>, [<cid>]
            */
            /* TODO: get CID ? we could used this cid if problem with <PDP_addr> occured */
        }
        else
        {
             PrintDBG("+CGEV parameter rank %d ignored", element_infos->param_rank);
    ...
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_EXECUTION_CMD)
    {
        ...
        */
        ...
        if (element_infos->param_rank == 2)
        {
            ...
            p_modem_ctxt->SID_ctxt.signal_quality->rssi = rssi;
        }
        if (element_infos->param_rank == 3)
        {
            ...
            p_modem_ctxt->SID_ctxt.signal_quality->ber = ber;
        }
    }
    ...
    if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD)
    {
        ...
        */
        ...
        if (element_infos->param_rank == 2)
        {
            ...
            PrintDBG("+CGPADDR cid=%d", modem_cid);
            p_modem_ctxt->CMD_ctxt.modem_cid = modem_cid;
        }
        else if ((element_infos->param_rank == 3) || (element_infos->param_rank == 4))
        {
