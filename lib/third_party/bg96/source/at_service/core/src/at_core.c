...
#elif (USE_CONFIGPRINTF_ATCORE == 1)
#define PrintINFO(format, args...)  vLoggingPrintf("ATCore:" format "\r\n", ## args)
#define PrintDBG(format, args...)   vLoggingPrintf("ATCore:" format "\r\n", ## args)
#define PrintAPI(format, args...)   vLoggingPrintf("ATCore API:" format "\r\n", ## args)
#define PrintErr(format, args...)   vLoggingPrintf("ATCore ERROR:" format "\r\n", ## args)
#else
...
/* signals definition for IPC message queue */
#define SIG_IPC_MSG  (1 << 0)
...
static uint8_t AT_Core_initialized = 0;
...
/* array of IPC Messages (1 element per ATCore handler */
static IPC_RxMessage_t     msgFromIPC[ATCORE_MAX_HANDLES];
/* array of msg received counters (1 element per ATCore handler */
static __IO uint8_t        MsgReceived[ATCORE_MAX_HANDLES] = {0};
...
at_status_t  AT_init(void)
{
    ...
    for( idx=0; idx<ATCORE_MAX_HANDLES; idx++)
    {
        ...
        at_context[idx].processing_cmd = 0;
...
at_handle_t  AT_open(sysctrl_info_t *p_device_infos, event_callback_t event_callback, urc_callback_t urc_callback)
{
    ...
    at_context[affectedHandle].processing_cmd = 0;
...
at_status_t AT_reset_context(at_handle_t athandle)
{
    ...
    at_context[athandle].processing_cmd = 0;
...
at_status_t AT_sendcmd(at_handle_t athandle, at_msg_t msg_in_id, at_buf_t *p_cmd_in_buf, at_buf_t *p_rsp_buf)
{
    ...
    */
    ...
    if (at_context[athandle].processing_cmd == 1)
    {
    ...
    at_context[athandle].processing_cmd = 1;
    ...
    if (at_context[athandle].in_data_mode == AT_TRUE)
    {
        ...
        if (msg_in_id == SID_CS_DATA_SUSPEND)
        {
            ...
            PrintINFO("<<< restore IPC COMMAND channel 0x%x >>>", &ipcHandleTab[athandle]);
    ...
    {
        PrintErr("AT_sendcmd error: process request");
    ...
    if (retval != ATSTATUS_OK)
    {
        PrintErr("AT_sendcmd error: process AT transaction");
        ATParser_abort_request(&at_context[athandle]);
        goto exit_func;
    }
...
at_status_t  AT_getevent(at_handle_t athandle, at_buf_t *p_rsp_buf)
{
    ...
    {
        /* TODO use PrintDBG instead of PrintErr */
        PrintErr("this is not an URC (%d)", action);
...
static uint8_t find_index(IPC_Handle_t *ipcHandle)
{
    uint8_t idx = 0, found = 0, retval = 0;
    ...
    do
    {
        ...
        idx++;
    } while ((idx <= ATCORE_MAX_HANDLES ) && (found == 0));
...
static void msgReceivedCallback(IPC_Handle_t *ipcHandle)
{
    ...
    if(osMessagePut (q_msg_IPC_received_Id, SIG_IPC_MSG, 0) != osOK)
    {
...
#endif /* DBG_REQUEST_DURATION */
    PrintDBG("**** Waiting Sema (to=%u) *****", Timeout);
...
static at_status_t process_AT_transaction(at_handle_t athandle, at_msg_t msg_in_id, at_buf_t *p_rsp_buf)
{
    ...
    do
    {
        ...
        /* Get command to send */
        action_send = ATParser_get_ATcmd(&at_context[athandle], (uint8_t*)&build_atcmd[0], &build_atcmd_size, &at_cmd_timeout);
        ...
        if ((action_send & ATACTION_SEND_WAIT_MANDATORY_RSP) || (action_send & ATACTION_SEND_TEMPO))
        {
            ...
            do
            {
                ...
                if (retval != ATSTATUS_OK)
                {
                    if (action_send & ATACTION_SEND_WAIT_MANDATORY_RSP)
                    {
                    ...
                    else /* ATACTION_SEND_TEMPO */
                    {
                        ...
                        */
                        if (action_send & ATACTION_SEND_FLAG_LAST_CMD)
                        {
                ...
                */
                if (at_context[athandle].action_flags & ATACTION_RSP_FRC_END)
                {
                    action_rsp = ATACTION_RSP_FRC_END;
                    /* clean flag */
                    at_context[athandle].action_flags &= ~((at_action_rsp_t) ATACTION_RSP_FRC_END);
                }
                else if (at_context[athandle].action_flags & ATACTION_RSP_FRC_CONTINUE)
                {
                    action_rsp = ATACTION_RSP_FRC_CONTINUE;
                    /* clean flag */
                    at_context[athandle].action_flags &= ~((at_action_rsp_t) ATACTION_RSP_FRC_CONTINUE);
                }
                else if (at_context[athandle].action_flags & ATACTION_RSP_ERROR)
                {
            ...
            {
                another_cmd_to_send = 1;
            }
            else
            {
                /* FRC_END, ERRORS,... */
                another_cmd_to_send = 0;
            }
        }
        else
        {
...
static at_action_rsp_t analyze_action_result(at_handle_t athandle, at_action_rsp_t val)
{
    ...
    uint8_t data_mode = ((val & ATACTION_RSP_FLAG_DATA_MODE) != 0);
...
at_status_t atcore_task_start(osPriority taskPrio, uint32_t stackSize)
{
  ...
  /* start driver thread */
  osThreadDef(ATCT, ATCoreTaskBody, taskPrio, 0, ATCORE_THREAD_STACK_SIZE);
  atcoreTaskId = osThreadCreate(osThread(ATCT), NULL);
  ...
  else
  {
            ...
            stackAnalysis_addThreadStackSizeByHandle(atcoreTaskId, stackSize);
...
static at_status_t findMsgReceivedHandle(at_handle_t* athandle)
{
    for(uint32_t i=0; i<ATCORE_MAX_HANDLES; i++)
    {
        if (MsgReceived[i] != 0)
        {
            *athandle = i;
