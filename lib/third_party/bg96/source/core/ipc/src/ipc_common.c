...
/* TODO list:
- replace Abort mechanism by official HAL abort added since beginning of IPC dev
- SPI to add
- use IPC_config file : use generic uart ptr defined in this file, same for uart config
- manage IPC_ZEROCOPY : add a function to free a message when it has been consumed by the client
- if message size received > buffer size : impossible to receive it : deadlock !! => full buffer indication
- ADD A BUFFER FULL INDICATION TO AVOID BLOCKING CASE (if no end char detected)
*/
