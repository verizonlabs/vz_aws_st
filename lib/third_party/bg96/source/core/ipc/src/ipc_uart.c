/**
  ******************************************************************************
  * @file    Application\core\ipc\src\ipc_uart.c
                           ...
                           IPC_CheckEndOfMsgCallbackTypeDef pCheckEndOfMsg)
{
    ...
    PrintDBG("IPC channel 0x%x registered", g_IPC_Devices_List[device].h_current_channel);
    PrintDBG("state 0x%x", g_IPC_Devices_List[device].state);
    PrintDBG("active channel handle: 0x%x", g_IPC_Devices_List[device].h_current_channel);
    PrintDBG("inactive channel handle: 0x%x", g_IPC_Devices_List[device].h_inactive_channel);
    ...
    /* start RX IT */
    uart_status = HAL_UART_Receive_IT(hipc->Interface.h_uart, (uint8_t*)g_IPC_Devices_List[device].RxChar, 1);
...
IPC_Status_t IPC_close_uart(IPC_Handle_t *hipc)
{
    ...
    PrintDBG("IPC channel 0x%x closed", hipc);
    PrintDBG("state 0x%x", g_IPC_Devices_List[hipc->Device_ID].state);
    PrintDBG("active channel handle: 0x%x", g_IPC_Devices_List[hipc->Device_ID].h_current_channel);
    PrintDBG("inactive channel handle: 0x%x", g_IPC_Devices_List[hipc->Device_ID].h_inactive_channel);
    return(IPC_OK);
}
    ...
    if (hipc != NULL)
    {
        ...
        if (device_id != IPC_DEVICE_NOT_FOUND)
        {
            ...
            */
            ...
            /* rearm IT */
            HAL_UART_Receive_IT(hipc->Interface.h_uart, (uint8_t*)g_IPC_Devices_List[device_id].RxChar, 1);
            hipc->State = IPC_STATE_ACTIVE;
            retval = IPC_OK;
        }
    }
...
void IPC_dump_RX_queue_uart(IPC_Handle_t *hipc, uint8_t readable)
{
    ...
    */
    ...
    while ((header.complete == 1) && (dump_index != first_uncomplete_header))
    {
...
static uint8_t find_Device(UART_HandleTypeDef *huart)
{
    ...
    for (device_id=0; device_id < IPC_MAX_DEVICES; device_id++)
    {
